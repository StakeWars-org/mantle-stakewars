           ...
           if (!updateStatsResponse.ok) {
             try {
               ...
               if (contentType && contentType.includes("application/json")) {
                 ...
                 // Log but continue with claim flow - stats update failure shouldn't block claiming
                 console.warn("⚠️ Stats update failed, but continuing with claim flow");
               } else {
                 console.warn(`⚠️ Stats update failed with HTTP ${updateStatsResponse.status}, but continuing with claim flow`);
               }
             } catch (parseError) {
               console.error("❌ Failed to parse error response:", parseError);
               console.warn("⚠️ Stats update failed, but continuing with claim flow");
             }
             // Continue with claim flow even if stats update fails
           } else {
             ...
             try {
               ...
               if (statsData.success) {
                 ...
                 console.warn("⚠️ Stats update returned success:false, but continuing with claim flow");
               }
             } catch (parseError) {
    ...
    <div className="bg-[#191919]/60 h-full w-full top-0 left-0">
      <div className="flex flex-col justify-center items-center h-full">
        ...
        <div className="flex flex-col justify-center items-center gap-4 -mt-48">
          ...
          >
            {isClaiming ? "Claiming..." : hasClaimed ? "Claimed!" : "Claim 5 XP + 50 Chakra"}
          </Button>
        </div>
      </div>
    </div>
