...
export default function CombatFeedback({ gameState }: CombatFeedbackProps) {
  const [damageNumbers, setDamageNumbers] = useState<DamageNumber[]>([]);
  const [abilityUsed, setAbilityUsed] = useState<{ name: string; player: 'player' | 'ai' } | null>(null);

  useEffect(() => {
    if (gameState.lastAttack) {
      const attack = gameState.lastAttack;
      const damage = attack.actualDamage || attack.ability.value;
      const isCritical = damage > (attack.ability.value + 2); // Approximate critical detection
      
      // Add damage number
      const newDamage: DamageNumber = {
        id: Date.now().toString(),
        value: damage,
        x: attack.attackingPlayer === 'player' ? 75 : 25, // Position based on attacker
        y: 50,
        isCritical,
        target: attack.attackingPlayer === 'player' ? 'ai' : 'player'
      };
    ...
    <>
        ...
        {damageNumbers.map((damage) => (
          <div
            ...
            className={`absolute transform -translate-x-1/2 -translate-y-1/2 animate-bounce ${
              damage.target === 'ai' ? 'left-[25%]' : 'left-[75%]'
            }`}
            style={{
              top: `${damage.y}%`,
      ...
      {abilityUsed && (
        <div className={`absolute top-10 left-1/2 transform -translate-x-1/2 z-30 animate-pulse ${
          abilityUsed.player === 'player' ? 'text-[#BFE528]' : 'text-red-400'
        }`}>
          <div className="bg-black/80 backdrop-blur-sm px-6 py-3 rounded-lg border-2 border-current">
            <div className="text-lg font-bold text-center">
              {abilityUsed.player === 'player' ? 'You' : 'AI'} used: {abilityUsed.name}
            </div>
          </div>
        </div>
      )}
